...
nrg_data<-nrg_data%>%
  ...
  #adjusting region names to fit with data
  mutate(Region_nominal = case_when(
    Region_nominal == "Bayern" ~ "Bavaria",
    Region_nominal == "Bourgogne-Franche-Comté" ~ "Bourgogne-Franche-Comte",
    Region_nominal == "Lombardia" ~ "Lombardy",
    Region_nominal == "Hessen" ~ "Hesse",
    Region_nominal == "Mecklenburg-Vorpommern" ~ "Mecklenburg-Western Pomerania",
    Region_nominal == "Occitanie" ~ "Occitania",
    Region_nominal == "Piemonte" ~ "Piedmont",
    Region_nominal == "Provence-Alpes-Côte d’Azur" ~ "Provence-Alpes-Côte d'Azur",
    Region_nominal == "Rheinland-Pfalz" ~ "Rhineland-Palatinate",
    Region_nominal == "Rīga" ~ "Riga",
    Region_nominal == "Sardegna" ~ "Sardinia",
    Region_nominal == "Sachsen" ~ "Saxony",
    Region_nominal == "Thüringen" ~ "Thuringen",
    Region_nominal == "Hovedstaden" ~ "Capital Region of Denmark",
    Region_nominal == "Syddanmark" ~ "Region of Southern Denmark",
    Region_nominal == "Sjælland" ~ "Zealand Region",
    Region_nominal == "Nordjylland" ~ "North Denmark Region",
    Region_nominal == "Midtjylland" ~ "Central Denmark Region",
    Region_nominal == "Niedersachsen" ~ "Lower Saxony",
    Region_nominal == "Nordrhein-Westfalen" ~ "North Rhine-Westphalia",
    Region_nominal == "Sachsen-Anhalt" ~ "Saxony-Anhalt",
    Region_nominal == "Ile de France" ~ "Paris Region",
    Region_nominal == "Centre — Val de Loire" ~ "Centre - Val de Loire",
    Region_nominal == "Nouvelle-Aquitaine" ~ "Nouvelle Aquitaine",
    Region_nominal == "Puglia" ~ "Apulia",
    Region_nominal == "Sicilia" ~ "Sicily",
    Region_nominal == "Hauts-de-France" ~ "Hauts-de France",
    Region_nominal %in% c("Provincia Autonoma di Trento") ~ "Trentino - Alto Adige / Sudtirol",
    Region_nominal == "Toscana" ~ "Tuscany",
    TRUE ~ Region_nominal
  ))
...
clean_basic_model <- unclean_model %>%
  ...
  mutate(var = case_when(grepl("factor\\(Region_nominal\\)", var) & country %in% c("Germany") ~ paste(gsub("factor\\(Region_nominal\\)", "", var),"(vs. Baden-Württemberg)"),
                         ...
                         T~var))%>%
  mutate(var = case_when(var == "Bourgogne-Franche-Comte (vs. Capital Region of Denmark)"~"Burgundy Franche-Comté (vs. Capital Region of Denmark)",
                         var == "Bretagne (vs. Capital Region of Denmark)"~"Brittany (vs. Capital Region of Denmark)",
                         var == "Corse (vs. Capital Region of Denmark)"~"Corsica (vs. Capital Region of Denmark)",
                         var == "Hauts-de France (vs. Capital Region of Denmark)"~"Hauts-de-France (vs. Capital Region of Denmark)",
                         var == "Normandie (vs. Capital Region of Denmark)"~"Normandy (vs. Capital Region of Denmark)",
                         var == "Bretagne (vs. Capital Region of Denmark)"~"Brittany (vs. Capital Region of Denmark)",
                         
                         var == "Bourgogne-Franche-Comte (vs. Auvergne-Rhône-Alpes)"~"Burgundy Franche-Comté (vs. Auvergne-Rhône-Alpes)",
                         var == "Bretagne (vs. Auvergne-Rhône-Alpes)"~"Brittany (vs. Auvergne-Rhône-Alpes)",
                         var == "Corse (vs. Auvergne-Rhône-Alpes)"~"Corsica (vs. Auvergne-Rhône-Alpes)",
                         var == "Hauts-de France (vs. Auvergne-Rhône-Alpes)"~"Hauts-de-France (vs. Auvergne-Rhône-Alpes)",
                         var == "Normandie (vs. Auvergne-Rhône-Alpes)"~"Normandy (vs. Auvergne-Rhône-Alpes)",
                         var == "Bretagne (vs. Auvergne-Rhône-Alpes)"~"Brittany (vs. Auvergne-Rhône-Alpes)",
                         
                         var == "Thuringen (vs. Capital Region of Denmark)"~"Thuringia (vs. Capital Region of Denmark)",
                         var == "Thuringen (vs. Baden-Württemberg)"~"Thuringia (vs. Baden-Württemberg)",
                         
                         var == "Trentino - Alto Adige / Sudtirol (vs. Capital Region of Denmark)"~"Trentino - South Tyrol (vs. Capital Region of Denmark)",
                         var == "Trentino - Alto Adige / Sudtirol (vs. Abruzzo)"~"Trentino - South Tyrol (vs. Abruzzo)",
                         T~var))
...
data_subset<-data_subset %>%
  arrange(var = factor(var, levels = c(
    ...
    "Occitania (vs. Capital Region of Denmark)",
